...
const restartState = new Map<
  string,
  { failures: number; nextRetry: number; gaveUp: boolean }
>();
...
function calculateNextIntervalRun(
  ...
  const next = new Date(
    now.getTime() + baseMs + getRandomJitterMs(jitterMinutes),
  );
...
async function runPluginJob(plugin: DiscoveredPlugin): Promise<void> {
  ...
  const state = schedulerState.get(plugin.manifest.id) || {
    running: false,
    nextRun: null,
  };
...
async function checkExistingProcess(
  ...
  try {
    ...
    // Use `-a` so we can inspect full command lines and ignore pgrep itself.
    // Otherwise `pgrep ... | head -1` can return pgrep's transient PID.
    const { stdout } = await execAsync(`pgrep -af "${searchPattern}"`);
    const lines = stdout
      .split("\n")
      .map((line) => line.trim())
      .filter(Boolean);
    ...
    for (const line of lines) {
      ...
      if (cmd.includes("pgrep -af")) continue;
      if (!cmd.includes(searchPattern)) continue;
      return pid;
    }
  } catch {
...
async function ensurePluginServer(plugin: DiscoveredPlugin): Promise<void> {
  ...
  if (existingPid) {
    ...
    if (knownExternalPids.get(pluginId) !== existingPid) {
      console.log(
        `ℹ️ ${plugin.manifest.name} server already running externally (PID ${existingPid})`,
      );
  ...
  child.on("exit", (code) => {
    ...
    const state = restartState.get(pluginId) || {
      failures: 0,
      nextRetry: 0,
      gaveUp: false,
    };
    ...
    if (uptime < FAST_CRASH_THRESHOLD_MS) {
      ...
      if (state.failures >= MAX_FAST_FAILURES) {
        ...
        console.error(
          `❌ ${plugin.manifest.name} server crashed ${state.failures} times quickly — giving up. Start it manually.`,
        );
        return;
      }
      const delay = Math.min(
        5_000 * Math.pow(2, state.failures - 1),
        MAX_BACKOFF_MS,
      );
    ...
    void (async () => {
      ...
      if (schedule.autoStartServer && schedule.autoRestartServer) {
        ...
        if (delay > 0) {
          console.log(
            `🔄 Restarting ${plugin.manifest.name} server in ${Math.round(delay / 1000)}s (attempt ${state.failures}/${MAX_FAST_FAILURES})`,
          );
        }
        setTimeout(() => ensurePluginServer(plugin), delay || 5000);
      } else if (schedule.autoStartServer && !schedule.autoRestartServer) {
        console.log(
          `⏸️ ${plugin.manifest.name} server stopped (auto-restart disabled)`,
        );
      }
    })();
  });
...
function parseFixedTimes(times: string[]): Set<string> {
  const normalized = times.map((time) => time.trim()).filter(Boolean);
...
async function tick(plugins: DiscoveredPlugin[]): Promise<void> {
  ...
  for (const plugin of plugins) {
    ...
    if (schedule.autoStartServer && plugin.manifest.commands.server) {
    ...
    if (schedule.cadence === "fixed") {
      ...
      if (
        fixedTimes.has(now.toTimeString().slice(0, 5)) &&
