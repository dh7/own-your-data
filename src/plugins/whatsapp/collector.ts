...
function disconnectReasonLabel(statusCode?: number): string {
  if (statusCode == null) return "unknown";
  const entries = Object.entries(DisconnectReason) as Array<[string, number]>;
...
function formatErrorDetail(err: unknown): string {
  ...
  if (err instanceof Error) {
    ...
    return `${err.name}: ${firstStackLine}`;
  }
...
async function processMessages(
  ...
  try {
    const config = await import("../../config/config").then((m) =>
      m.loadConfig(),
    );
...
export async function collectRawMessages(
  ...
  process.on("uncaughtException", (err) => {
    console.error("❌ Uncaught exception in WhatsApp collector:", err);
  });
  process.on("unhandledRejection", (reason) => {
    console.error("❌ Unhandled rejection in WhatsApp collector:", reason);
  });
  ...
  const createSocket = async (): Promise<CollectorStats> => {
    ...
    return new Promise((resolve, reject) => {
      ...
      sock.ev.on("messaging-history.set" as any, async (data: any) => {
        console.log(
          `📥 History: ${data.chats?.length || 0} chats, ${data.messages?.length || 0} messages`,
        );
      ...
      sock.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        const result = await saveRawDump(rawDumpsDir, `upsert-${type}`, {
          messages,
          type,
        });
        stats.messageCount += result.messageCount;
      ...
      sock.ev.on("connection.update", async (update) => {
        ...
        console.log(
          `🔎 connection.update: state=${connection || "unknown"} ` +
            `qr=${qr ? "present" : "none"} ` +
            `statusCode=${statusCode ?? "n/a"} reason=${reason}`,
        );

        if (qr) {
          console.log(
            '📱 QR Code required. Run "npm run config" to authenticate.',
          );
        ...
        if (connection === "close") {
          const errorDetail = formatErrorDetail(lastDisconnect?.error);
          console.error(`⚠️ Connection closed: ${errorDetail}`);

          if (statusCode === DisconnectReason.loggedOut) {
            console.error(
              '❌ Session logged out. Run "npm run config" to re-authenticate.',
            );
          ...
          if (reconnectAttempts <= MAX_RECONNECT_ATTEMPTS) {
            ...
            console.log(
              `📴 Connection closed. Reconnecting in ${delay / 1000}s... (attempt ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})`,
            );
            setTimeout(async () => {
              try {
