...
import {
  ...
  SchedulerPluginEditor,
} from "./templates/scheduler";
import {
  renderPluginsHub,
  wrapPluginPanel,
  PluginSummary,
} from "./templates/plugins";
...
async function checkPlaywright(): Promise<{
  installed: boolean;
  browsers: boolean;
}> {
  let installed = false;
...
async function checkInstagramAuth(
...
async function ensureServicePidDir(): Promise<void> {
  await fs.mkdir(path.join(process.cwd(), "logs", "services"), {
    recursive: true,
  });
}
...
async function startServiceByCommand(
  serviceId: string,
  command: string,
): Promise<void> {
...
function extractPluginStatus(renderedHtml: string): {
  ...
  const match = renderedHtml.match(
    /<span class="status\s+([^"]+)">([\s\S]*?)<\/span>/i,
  );
  if (!match) {
  ...
  const classes = match[1]
    ...
    .filter(Boolean);
  const className =
    (["connected", "disconnected", "pending", "warning"] as const).find((cls) =>
...
async function startAutoStartPluginServers(): Promise<void> {
  ...
  for (const plugin of plugins) {
    if (
      !plugin.manifest.commands.server ||
      plugin.manifest.commands.server.trim().length === 0
    ) {
      continue;
    }
    const scheduler = await resolvePluginSchedule(plugin);
    if (!scheduler.enabled || !scheduler.autoStartServer) continue;
    await startServiceByCommand(
      getPluginServiceId(plugin.manifest.id),
      plugin.manifest.commands.server,
    );
  }
}
...
function githubSshToHttps(url: string): string | null {
  const scpStyle = url.match(/^git@github\.com:(.+)$/);
  if (scpStyle) {
    return `https://github.com/${scpStyle[1]}`;
  }
  const sshStyle = url.match(/^ssh:\/\/git@github\.com\/(.+)$/);
  if (sshStyle) {
    return `https://github.com/${sshStyle[1]}`;
  }
  return null;
}
  ...
  try {
    ...
    if (!skipFetch) {
      ...
      try {
        ...
        // Fallback for GitHub SSH remotes: attempt read-only fetch over HTTPS.
        const fallbackUrl = githubSshToHttps(remoteUrl);
        if (fallbackUrl) {
          try {
            await execAsync(`git fetch ${fallbackUrl} main --quiet`, {
              cwd: process.cwd(),
              timeout: 8000,
              env: {
                ...process.env,
            ...
            fetchSucceeded = true;
            fetchError = "";
            console.log("✅ Git fetch succeeded via HTTPS fallback");
            ...
            console.log(
              "⚠️ Git fetch failed (origin + HTTPS fallback) - using cached data",
            );
            fetchSucceeded = false;
          }
        } else {
          console.log(
            "⚠️ Git fetch failed (auth required or timeout) - using cached data",
          );
          fetchSucceeded = false;
        }
      }
    ...
    return {
      ...
      commitsBehind: 0,
      fetchSucceeded: false,
      fetchError: String((e as any)?.message || e || "unknown error"),
    };
  }
}
...
app.get("/", async (req, res) => {
  ...
  const savedSection =
    rawSavedSection === "daemon" || rawSavedSection === "scheduler"
      ? "services"
      : rawSavedSection;
  ...
  const nvidiaDockerInstalled = dockerInstalled
    ? await checkNvidiaDocker()
    : false;
  ...
  for (const discovered of plugins) {
    ...
    const pluginConfig =
      (await loadPluginConfig(discovered.manifest.id)) ||
    ...
    const hasServer =
      typeof discovered.manifest.commands.server === "string" &&
      discovered.manifest.commands.server.trim().length > 0;
  ...
  const schedulerServices: SchedulerServiceStatus[] = [
    ...
    },
    {
      ...
      description: tunnelStatus.tunnelConfigured
        ? "Standalone service (npm run tunnel)"
        : "Not configured",
      detail: tunnelStatus.tunnelRunning
        ? tunnelStatus.tunnelUrl || `Proxy on port ${PROXY_PORT}`
        : tunnelStatus.tunnelConfigured
          ? "Ready to start"
          : "Configure in Domain section",
  ...
  for (const plugin of plugins) {
    ...
    if (!running && plugin.manifest.tunnel?.enabled) {
      const healthRoute = plugin.manifest.tunnel.routes.find(
        (route) => route.path === "/health" && route.auth === false,
      );
      if (healthRoute) {
        running = await checkServiceHealth(
          `http://127.0.0.1:${plugin.manifest.tunnel.port}${healthRoute.path}`,
        );
      }
    }
  ...
  const system = renderSystemSection(
    config,
    {
  ...
  const allModals =
    services.modal + system.modals + domain.modal + files.modal + github.modal;
  ...
  const initialPluginPanel = pluginSummaries.some((p) => p.id === savedSection)
    ? savedSection
    : undefined;

  // Generate scheduler popup panels for each plugin
  const schedulerPanels = pluginScheduleRows.map((p) =>
    renderSchedulerPanel(p),
  );
...
app.post("/scheduler/plugin/:id", async (req, res) => {
  ...
  const fixedTimes =
    typeof req.body.fixedTimes === "string"
      ? req.body.fixedTimes
          .split(",")
  ...
  const nextConfig: PluginSchedule = {
    ...
    startHour: Math.min(
      23,
      Math.max(0, parseInt(req.body.startHour, 10) || existing.startHour),
    ),
    endHour: Math.min(
      24,
      Math.max(1, parseInt(req.body.endHour, 10) || existing.endHour),
    ),
    intervalHours: Math.min(
      168,
      Math.max(
        1,
        parseInt(req.body.intervalHours, 10) || existing.intervalHours,
      ),
    ),
    jitterMinutes: Math.min(
      180,
      Math.max(
        0,
        parseInt(req.body.jitterMinutes, 10) || existing.jitterMinutes,
      ),
    ),
    fixedTimes,
...
app.post("/scheduler/run/:pluginId/:command", async (req, res) => {
  ...
  if (!["get", "process", "push"].includes(command)) {
    res
      ...
      .json({
        success: false,
        error: "Invalid command. Must be get, process, or push.",
      });
    return;
  }
  ...
  try {
    ...
    if (!plugin) {
      res
        .status(404)
        .json({ success: false, error: `Plugin not found: ${pluginId}` });
      return;
    }

    const cmd =
      plugin.manifest.commands[
        command as keyof typeof plugin.manifest.commands
      ];
    if (!cmd || cmd.trim().length === 0) {
      res
        ...
        .json({
          success: false,
          error: `Plugin ${pluginId} has no "${command}" command.`,
        });
      return;
    }
...
app.get("/plugin/logs/:pluginId", async (req, res) => {
  ...
  const maxLines = Math.min(
    Math.max(parseInt(req.query.lines as string, 10) || 500, 10),
    5000,
  );
...
app.post("/plugin/:id", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ ${plugin.manifest.name} config saved to config/${pluginId}.json`,
    );
...
app.get("/chrome-extension-configured", async (req, res) => {
  ...
  try {
    ...
    const zipPath = path.join(
      basePath,
      "logs",
      "chrome-extension-configured.zip",
    );
    ...
    const tunnelConfigPath = path.join(
      basePath,
      "auth",
    ...
    try {
      const tunnelConfig = JSON.parse(
        await fs.readFile(tunnelConfigPath, "utf8"),
      );
...
`;
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="chrome-extension-configured.zip"',
    );
...
app.post("/files/delete", async (req, res) => {
  try {
    ...
    if (
      protectedPaths.some(
        (p) => fullPath.endsWith(p) || fullPath.includes("/node_modules/"),
      )
    ) {
...
app.post("/files/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    console.log(
      `📁 File uploaded: ${req.file.filename} → ${req.file.destination}/${req.file.filename}`,
    );
    res.json({
      success: true,
...
app.post("/dependencies/install-syncthing", async (req, res) => {
  ...
  try {
    ...
    if (platform === "darwin") {
      ...
      installCmd =
        "sudo apt-get install -y syncthing || sudo snap install syncthing";
    } else {
      res.json({
        success: false,
        error:
          "Automatic installation not supported on this platform. Please install Syncthing manually from https://syncthing.net/downloads/",
      });
      return;
    }
  ...
  try {
    ...
    try {
      ...
      res.json({
        success: false,
        error:
          "Syncthing config not found. Please run Syncthing at least once first.",
      });
      return;
    }
    ...
    if (configContent === originalContent) {
      res.json({
        success: false,
        error:
          "Could not find GUI address to update in config. Config format may have changed.",
      });
      return;
    }
    ...
    try {
      ...
      if (platform === "linux") {
        await execAsync("nohup syncthing > /dev/null 2>&1 &", {
          timeout: 5000,
        });
      } else {
    ...
    res.json({
      success: true,
      message:
        "Syncthing configured for remote access!" +
        restartMessage +
        " GUI now accessible on 0.0.0.0:8384",
    });
  } catch (e: any) {
...
app.post("/system/pull-updates", async (req, res) => {
  ...
  try {
    ...
    const { stdout: npmOutput, stderr: npmStderr } = await execAsync(
      "npm install",
      {
        cwd: process.cwd(),
    ...
    res.json({
      success: true,
      message:
        "Updates pulled and dependencies installed! Restart the server to apply changes.",
      output:
        pullOutput +
        (pullStderr ? "\n" + pullStderr : "") +
        "\n--- npm install ---\n" +
...
app.post("/system/restart-config", async (req, res) => {
  ...
  res.json({
    success: true,
    message: "Config server will restart. Page will reload shortly.",
  });
...
app.post("/system/list-dirs", async (req, res) => {
  ...
  try {
    ...
    if (resolved.startsWith(cwd + "/") || resolved === cwd) {
      relativePath =
        resolved === cwd ? "." : "./" + resolved.slice(cwd.length + 1);
    }
...
app.post("/system/service/:id/:action", async (req, res) => {
  ...
  try {
    ...
    if (!plugin || !command || command.trim().length === 0) {
      res
        .status(404)
        .json({ success: false, error: "Service command not found" });
      return;
    }
    ...
    res.json({
      success: true,
      message: `${plugin.manifest.name} server ${action} complete`,
    });
  } catch (e: any) {
    res.json({ success: false, error: e.message });
  }
...
app.get("/system/logs/:serviceId", async (req, res) => {
  ...
  const maxLines = Math.min(
    Math.max(parseInt(req.query.lines as string, 10) || 200, 10),
    2000,
  );
...
app.post("/tunnel/test-chrome", async (req, res) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      res.json({
        success: false,
        message: "Tunnel reachable but API key mismatch",
      });
    } else {
...
app.post("/tunnel/save-credentials", async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: `Credentials saved! Connected to ${testResult.zoneName}`,
    });
  } catch (e: any) {
...
app.post("/tunnel/setup", async (req, res) => {
  ...
  if (!name || !subdomain) {
    res.json({
      success: false,
      message: "Tunnel name and subdomain are required",
    });
    return;
  }
  ...
  console.log(
    `☁️  Setting up tunnel "${name}" with subdomain "${subdomain}"...`,
  );
...
app.post("/tunnel/teardown", async (req, res) => {
  ...
  const result = await teardownTunnel(
    config.credentials,
    config.tunnelId,
    config.hostname,
  );
...
app.post("/test-github", async (req, res) => {
  ...
  try {
    ...
    if (putRes.ok) {
      res.json({
        success: true,
        message: `File created at ${owner}/${repo}/${filePath}`,
      });
    } else {
...
async function main() {
  ...
  const plugins = await discoverPlugins();
  console.log(
    `📦 Discovered ${plugins.length} plugins: ${plugins.map((p) => p.manifest.name).join(", ")}\n`,
  );
