...
function cleanCaption(caption: string): string {
    ...
    // Try to extract text after the colon (the actual caption)
    const colonMatch = caption.match(/:\s*"?(.+?)"?\.?\s*$/);
    if (colonMatch && colonMatch[1]) {
        return colonMatch[1].replace(/^"|"$/g, '').trim();
    }

    // No colon, try removing just the "username on Date" prefix
...
async function main() {
    ...
    const instagramOutputDir = paths.instagramLocal;
    const imagesOutputDir = path.join(instagramOutputDir, 'images');
    await fs.mkdir(instagramOutputDir, { recursive: true });
    ...
    for (const username of accounts) {
        ...
        const mdPath = path.join(instagramOutputDir, `instagram-${username}.md`);
    ...
    console.log(`\n✨ Done! Files saved to: ${instagramOutputDir}`);
}
